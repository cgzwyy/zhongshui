...
//        set2.setColor(ColorTemplate.getHoloBlue());
    ...
    private void tmpSetData() {
        ...
        new Thread() {
            public void run(){
                WebService.getInstance().GetStationP("17221","Gbe_F11All_Wh","Gbe_F10All_Wh", new RequestCallback() {
...
//        }
        ...
        if (!result.contains("{")) {
            Toast.makeText(getApplicationContext(), "获取数据失败：" + result, Toast.LENGTH_SHORT).show();
            return;
        }
